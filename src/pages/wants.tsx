...
interface WantsTransaction {
  ...
  date: string;
  category: string;
  type: string; // Added type to differentiate between expense and income
}
  ...
  useEffect(() => {
    ...
    const fetchWantsTransactions = async () => {
      ...
      if (session) {
        ...
        if (data) {
          setTransactions(data);
          setTotalSpent(data.reduce((sum, tx) => sum + tx.amount, 0));
        }
      }
      setLoading(false);
    };

    fetchWantsTransactions();
  }, []);
        ...
        <SidebarInset>
          ...
          <main className="container mx-auto p-6 space-y-6">
            <div className="flex justify-between items-center">
              <h1 className="text-2xl font-bold">Wants</h1>
              <Button onClick={() => setIsAddExpenseOpen(true)}>Add Expense</Button>
            </div>

            <Card>
              <CardHeader>
                <CardTitle>Wants Overview</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">₹{totalSpent.toFixed(2)}</div>
                <p className="text-muted-foreground">Total spent on wants</p>
              </CardContent>
            </Card>

            <Card>
              <CardHeader>
                <CardTitle>Wants Transactions</CardTitle>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
